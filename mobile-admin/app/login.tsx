...
type Screen = "splash" | "login" | "welcome"
type LoginResponse = { token: string; name: string; tenantSlug: string; tenantName: string }
    ...
    <View style={styles.splashRoot}>
      ...
      <View style={styles.splashCenter}>
        ...
        <View style={styles.splashBrand}>
          <Text style={styles.splashSub}>Admin</Text>
      ...
      <View style={styles.splashBottom}>
        ...
        <Text style={styles.splashFooter}>Acceso exclusivo para administradores</Text>
      </View>
    </View>
  )
}

function LoginFormScreen({ onSuccess }: { onSuccess: (name: string, tenantName: string) => void }) {
  const { login } = useAuth()
  ...
  async function handleLogin() {
    if (!phone.trim() || !password.trim()) { setError("Completa todos los campos"); return }
    setError(null); setLoading(true)
    try {
      const data = await apiRequest<LoginResponse>("/api/mobile/admin/auth/login", {
        method: "POST",
        body: { phone: phone.trim(), password: password.trim() },
      })
      await login(data.token, data.name, data.tenantSlug, data.tenantName)
      onSuccess(data.name, data.tenantName)
    } catch (e) {
  ...
  return (
    <View style={styles.loginRoot}>
      ...
      <View style={styles.loginHero}>
        <Bubble>
          <Text style={styles.bubbleText}>¡Hola! Ingresa tus datos de acceso</Text>
        </Bubble>
        <View style={{ marginTop: 4 }}>
          <Image source={{ uri: CAT2 }} style={{ width: 72, height: 72, borderRadius: 36 }} />
        </View>
      </View>
...
function WelcomeScreen({ name, tenantName }: { name: string; tenantName: string }) {
  ...
  return (
    <View style={styles.onboardRoot}>
      ...
      <View style={styles.onboardCenter}>
        <Bubble>
          <Text style={[styles.bubbleText, { fontSize: 20, fontWeight: "900" }]}>
            ¡Bienvenido, <Text style={styles.bubblePurple}>{name}</Text>! 🎉
          </Text>
          <Text style={[styles.bubbleText, { marginTop: 8, fontWeight: "600" }]}>
            Administra los alumnos de{" "}
            <Text style={styles.bubblePurple}>{tenantName}</Text> desde aquí 🚀
          </Text>
        </Bubble>
        <View style={{ marginTop: 4 }}>
      ...
      <View style={styles.onboardBottom}>
        <PurpleBtn label="IR AL PANEL" onPress={() => router.replace("/(tabs)")} />
      </View>
    </View>
  )
}
...
export default function LoginScreen() {
  ...
  const screens: Record<Screen, React.ReactNode> = {
    ...
    login:   <LoginFormScreen onSuccess={(name, tn) => { setAdminName(name); setTenantName(tn); goTo("welcome") }} />,
    welcome: <WelcomeScreen name={adminName} tenantName={tenantName} />,
  }
...
const styles = StyleSheet.create({
  ...
  splashRoot: {
    ...
    paddingTop: 72, paddingBottom: 48, paddingHorizontal: 32,
  },
  splashCenter: { alignItems: "center", gap: 20 },
  splashWelcome: { color: "#ffffff", fontSize: 48, fontWeight: "900", letterSpacing: -1.5, textAlign: "center" },
  splashBrand: { alignItems: "center", gap: 4 },
  splashSub: { color: "rgba(255,255,255,0.65)", fontSize: 15, fontWeight: "800", letterSpacing: 4, textTransform: "uppercase" },
  ...
  /* white button */
  whiteBtn: {
    width: "100%", paddingVertical: 18, borderRadius: 20, backgroundColor: "#ffffff",
    alignItems: "center",
    shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 0, elevation: 4,
  },
  whiteBtnText: { color: "#7c3aed", fontSize: 14, fontWeight: "900", letterSpacing: 1.5 },
  ...
  loginHero: { alignItems: "center", marginBottom: 20 },

  /* card */
  card: {
    backgroundColor: "#ffffff", borderRadius: 28, padding: 24, gap: 14,
    shadowColor: "#4c1d95", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.1, shadowRadius: 32,
  },
  ...
  bubbleBox: {
    backgroundColor: "#ffffff", borderRadius: 24, paddingHorizontal: 20, paddingVertical: 16,
    maxWidth: 300,
  ...
  bubbleTail: {
    ...
    borderTopColor: "#ffffff", alignSelf: "center",
  },
  ...
  /* onboard */
  onboardRoot: {
    flex: 1, backgroundColor: "#ffffff", alignItems: "center", justifyContent: "space-between",
    paddingTop: 72, paddingBottom: 48, paddingHorizontal: 24,
  },
